...
class TestSearchCapabilities(BaseTest):
    ...
    def test_single_text_index(self):
        try:
            self.data_collection.drop_indexes()
        except Exception:
            pass
        result_document = self.initialize_result_document("Single_Text_Index")
        with contextlib.redirect_stdout(self.__class__.stdout_stream), contextlib.redirect_stderr(self.__class__.stderr_stream):
            try:
                second_text_index = [('bio', TEXT)]
                self.data_collection.create_index(second_text_index, name='SecondTextIndex')
            except OperationFailure as e:
                result_document['errors'].append(str(e))
            else:
                result_document['status'] = 'pass'
    ...
    def test_regex_search_supported(self):
        result_document = self.initialize_result_document("Regex_Search_Supported")
        with contextlib.redirect_stdout(self.__class__.stdout_stream), contextlib.redirect_stderr(self.__class__.stderr_stream):
            try:
                regex_query = {'bio': {'$regex': 'CEO'}}
                results = list(self.data_collection.find(regex_query))
                result_document['command_result'] = {'regex_search_results': results}
                result_document['status'] = 'pass'
                result_document['exit_code'] = 0
                result_document['reason'] = 'PASSED'
            except OperationFailure as e:
                ...
                result_document['errors'].append(f'Exception during regex search: {e}')
            finally:
                result_document['warnings'] = [str(w.message) for w in self.__class__.warnings_list]
                result_document['stdout'] = self.__class__.stdout_stream.getvalue()
                result_document['stderr'] = self.__class__.stderr_stream.getvalue()
    ...
    def test_hint_supported_with_text(self):
        result_document = self.initialize_result_document("Hint_Supported_With_Text")
        with contextlib.redirect_stdout(self.__class__.stdout_stream), contextlib.redirect_stderr(self.__class__.stderr_stream):
            try:
                text_query = {'$text': {'$search': 'CEO'}}
                # Do not use hint() because text and hint are not allowed in the same query.
                results = list(self.data_collection.find(text_query))
                result_document['command_result'] = {'hint_search_results': results}
                result_document['status'] = 'pass'
                result_document['exit_code'] = 0
                result_document['reason'] = 'PASSED'
            except OperationFailure as e:
                ...
                result_document['errors'].append(f'Exception during hint search: {e}')
            finally:
                result_document['warnings'] = [str(w.message) for w in self.__class__.warnings_list]
                result_document['stdout'] = self.__class__.stdout_stream.getvalue()
                result_document['stderr'] = self.__class__.stderr_stream.getvalue()
    ...
    def test_text_search_with_projection(self):
        result_document = self.initialize_result_document("Text_Search_With_Projection")
        with contextlib.redirect_stdout(io.StringIO()) as stdout, contextlib.redirect_stderr(io.StringIO()) as stderr:
            try:
                text_query = {'$text': {'$search': 'CEO'}}
                projection = {'name': 1, '_id': 0}
                results = list(self.data_collection.find(text_query, projection))
                result_document['command_result'] = {'text_search_results': results}
                expected_names = {'Eugenia Lopez', 'Jessie Irwin'}
                result_names = set(doc['name'] for doc in results)
                if expected_names == result_names:
                ...
                result_document['errors'].append(f'Exception during text search with projection: {e}')
            finally:
                result_document['stdout'] = stdout.getvalue()
                result_document['stderr'] = stderr.getvalue()
                self.finalize_result_document(result_document)
    ...
    def test_text_search_with_score(self):
        result_document = self.initialize_result_document("Text_Search_With_Score")
        with contextlib.redirect_stdout(io.StringIO()) as stdout, contextlib.redirect_stderr(io.StringIO()) as stderr:
            try:
                text_query = {'$text': {'$search': 'CEO'}}
                projection = {'name': 1, 'score': {'$meta': 'textScore'}}
                cursor = self.data_collection.find(text_query, projection)
                ...
                result_document['errors'].append(f'Exception during text search with score: {e}')
            finally:
                result_document['stdout'] = stdout.getvalue()
                result_document['stderr'] = stderr.getvalue()
                self.finalize_result_document(result_document)
    ...
    def test_text_search_in_aggregation(self):
        result_document = self.initialize_result_document("Text_Search_In_Aggregation")
        with contextlib.redirect_stdout(io.StringIO()) as stdout, contextlib.redirect_stderr(io.StringIO()) as stderr:
            try:
                match_stage = {'$match': {'$text': {'$search': 'CEO'}}}
                project_stage = {'$project': {'name': 1, 'score': {'$meta': 'textScore'}}}
                sort_stage = {'$sort': {'score': {'$meta': 'textScore'}}}
                pipeline = [match_stage, project_stage, sort_stage]
                results = list(self.data_collection.aggregate(pipeline))
                result_document['command_result'] = {'text_search_results': results}
                expected_names = {'Eugenia Lopez', 'Jessie Irwin'}
                result_names = set(doc['name'] for doc in results)
                ...
                result_document['errors'].append(f'Exception during text search in aggregation: {e}')
            finally:
                result_document['stdout'] = stdout.getvalue()
                result_document['stderr'] = stderr.getvalue()
                self.finalize_result_document(result_document)
    ...
    def test_text_search_with_language_option(self):
        result_document = self.initialize_result_document("Text_Search_With_Language_Option")
        with contextlib.redirect_stdout(io.StringIO()) as stdout, contextlib.redirect_stderr(io.StringIO()) as stderr:
            try:
                text_query = {'$text': {'$search': 'CEO', '$language': 'english'}}
                results = list(self.data_collection.find(text_query))
                result_document['command_result'] = {'text_search_results': results}
                expected_names = {'Eugenia Lopez', 'Jessie Irwin'}
                result_names = set(doc['name'] for doc in results)
                ...
                result_document['errors'].append(f'Exception during text search with language option: {e}')
            finally:
                result_document['stdout'] = stdout.getvalue()
                result_document['stderr'] = stderr.getvalue()
                self.finalize_result_document(result_document)
