...
class TestUserManagementCommands(BaseTest):
    ...
    def setUp(self):
        ...
        # Create the test user. Instead of raising an exception when the database returns an error,
        # record the error in a result document and continue.
        try:
            user_definition = {
            ...
            error_msg = f"Failed to create test user in setUp: {e}"
            self.logger.error(error_msg)
            # Record the error in a result document so it appears in our test results
            result_document = self.create_result_document("createUser")
            result_document['description'].append(error_msg)
            result_document['reason'] = 'FAILED'
            self.test_results.append(result_document)
            # Set test_user to None to indicate failure
            self.test_user = None
    ...
    def execute_and_store_command(self, command):
        ...
        try:
            if command == 'createUser':
                ...
                host_info = self.docdb_client.address  # (host, port)
                host, port = host_info
            ...
            end_time = time.time()
            start_dt = datetime.strptime(result_document['start'], "%Y-%m-%dT%H:%M:%S.%f")
            result_document['elapsed'] = end_time - start_dt.timestamp()
            ...
            self.test_results.append(result_document)
            # Fail the test if the status is not pass
            self.assertEqual(result_document['status'], 'pass',
                             f"User management command '{command}' failed: {result_document['description']}")
    ...
    def tearDown(self):
        # Clean up by attempting to drop the test user (if it exists)
        try:
            if self.test_user:
